...
REVOCABLE_TYPES = [TYPE_POA, TYPE_MCHD]
...
# Из каких статусов инициатор может ПРАВИТЬ поля заявки. Совпадает с теми, из
# которых её можно отправить на согласование: вернули на доработку — значит
# дорабатывать; на круге и после передачи исполнителю карточка заморожена,
# иначе согласующие визировали бы один текст, а юристы получали другой.
EDITABLE_STATUSES = [STATUS_DRAFT, STATUS_RETURNED, STATUS_REJECTED]

# Из каких статусов доверенность можно отозвать: она уже прошла согласование и
# либо оформляется юристами, либо выдана. Черновик и заявку на согласовании
# отзывать нечего — их отменяют (cancel), а не отзывают.
REVOCABLE_STATUSES = [
    STATUS_APPROVED, STATUS_TO_LEGAL, STATUS_LEGAL_WORK, STATUS_SIGNING,
...
# --- Анкета отзыва доверенности/МЧД -----------------------------------------
# Отзыв — короткая заявка: что отзываем (карточка доверенности либо реквизиты
# бумажной, выданной до MiniSED), почему и с какой даты. Полномочия, паспорт и
# прочая анкета представителя здесь не нужны — они уже есть в отзываемой
# доверенности.
ANKETA_REVOKE_REASONS = [
...
# Что именно отзываем, когда карточки в системе нет (бумажная доверенность,
# выданная до внедрения) — вид указывает инициатор руками.
ANKETA_REVOKE_KINDS = [
    (TYPE_POA, "Доверенность (бумажная)"),
    (TYPE_MCHD, "МЧД (машиночитаемая)"),
]
