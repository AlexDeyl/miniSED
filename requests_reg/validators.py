...
def is_gosuslugi(data) -> bool:
    ...
    На Госуслугах машиночитаемую доверенность выпускают по одному лишь ФИО и
    должности: ни паспорта, ни даты рождения, ни СНИЛС, ни ИНН, ни адреса
    регистрации там не спрашивают. Собирать их «на всякий случай» — лишние
    персональные данные в системе и лишние десять полей у инициатора, поэтому
    при этом флажке они не заполняются и в заявление не печатаются.
    """
    return bool((data or {}).get("gosuslugi"))
...
def mchd_rep_error(data) -> str | None:
    ...
    # Госуслуги: этих полей в анкете нет вовсе — проверять нечего.
    if is_gosuslugi(data):
        return None
