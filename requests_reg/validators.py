...
Госуслугах их для МЧД не требуют. Если инициатор их всё же заполнил — считаем
контрольные разряды, а не только длину: опечатка вскроется уже отказом ФНС,
а перестановка цифр — самая частая ошибка при вводе.
...
def mchd_rep_error(data) -> str | None:
    ...
    Поля НЕ обязательные: на Госуслугах их для МЧД не требуют, поэтому пустое
    значение — не ошибка. Но если инициатор их всё же заполнил, проверяем
    контрольные разряды: молча принять опечатку хуже, чем не иметь поля вовсе —
    вскроется она уже отказом ФНС.
